  ...
  return (
    <div role="alert" className="error-fallback">
      <div className="error-fallback-header">
        <h3>{componentName ? `Error in ${componentName}` : 'Something went wrong'}</h3>
        {resetErrorBoundary && (
      ...
      <div className="error-fallback-content">
        {error && (
          <div className="error-message">
            ...
            <div className="error-fallback-actions">
              <button onClick={() => setExpanded(!expanded)} className="error-details-toggle">
                {expanded ? 'Hide details' : 'Show details'}
              </button>

              {showReportButton && !reportSent && (
                <button onClick={handleReport} className="error-report-button">
                  Report issue
                </button>
              )}

              {reportSent && (
                <span className="error-report-sent">Thank you for reporting this issue</span>
              )}
            </div>
        ...
        {!error && <p>An unknown error occurred. Please try again or refresh the page.</p>}
      </div>
    </div>
  );
};
