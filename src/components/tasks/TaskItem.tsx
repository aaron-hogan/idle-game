      ...
      <div className={`task-item task-item--${task.status.toLowerCase()}`}>
        <div className="task-item__header">
          ...
          {task.completionCount > 0 && (
            <Tooltip content={`Completed ${task.completionCount} time${task.completionCount !== 1 ? 's' : ''}`}>
              <span className="task-item__completion-count">
        ...
        <div className="task-item__details">
              ...
              {Object.keys(task.cost).length > 0 ? (
                <ul>
                  {Object.entries(task.cost).map(([resourceId, amount]) => (
                    <li key={resourceId}>{resourceId}: {formatResourceAmount(amount)}</li>
                  ))}
                </ul>
              ) : (
                <p>No cost</p>
          ...
          <div className="task-item__duration">
            <h4>Duration:</h4>
            <p>{Math.floor(task.duration / 60)}:{(task.duration % 60).toString().padStart(2, '0')}</p>
          </div>
        </div>
