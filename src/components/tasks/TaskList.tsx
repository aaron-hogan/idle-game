...
import { selectAvailableTasks, selectTasksByCategory, selectActiveTask } from '../../state/tasksSlice';
...
const TaskList: React.FC = () => {
  ...
  const currentTasks = useSelector((state: RootState) => 
    activeCategory 
      ? selectTasksByCategory(state, activeCategory)
      : availableTasks
  );
    ...
    <ErrorBoundary fallback={<div className="task-list task-list--error">Error loading tasks</div>}>
      <div className="task-list">
        <div className="task-list__header">
          <h2>Tasks</h2>
          {activeTask && (
            <div className="task-list__active-task-info">
        ...
        <div className="task-list__content">
          {currentTasks.length > 0 ? (
            ...
            <p className="task-list__empty-category">
              No {activeCategory} tasks available yet.
            </p>
          )}
        </div>
      </div>
