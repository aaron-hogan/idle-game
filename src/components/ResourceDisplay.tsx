 ...
 */
const ResourceDisplay: React.FC<ResourceDisplayProps> = ({ resource, showDetails = true }) => {
  ...
  const formattedAmount =
    Math.floor(amount) === amount ? Math.floor(amount).toString() : amount.toFixed(1);

  // Format the per second rate with appropriate precision
  const formattedRate =
    perSecond === 0 ? '0' : perSecond < 0.1 ? perSecond.toFixed(2) : perSecond.toFixed(1);
  ...
  return (
    <div className="resource-display">
      ...
      <div className="resource-progress" title={`${capacityPercentage.toFixed(0)}% full`}>
        <div className="resource-progress-bar" style={{ width: `${capacityPercentage}%` }} />
      </div>
