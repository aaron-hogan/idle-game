...
const EventLog: React.FC<EventLogProps> = ({ maxEntries = 50 }) => {
  ...
  const filteredHistory =
    filter === 'all'
      ? eventHistory
      : eventHistory.filter((entry) => events[entry.eventId]?.type === filter);
    ...
    <div className="event-log">
      ...
      <div className="event-log__entries">
          ...
          return (
            ...
            >
              <div className="event-log__entry-header">
                ...
                <span className="event-log__entry-time">{formatDate(entry.timestamp)}</span>
              </div>

              <p className="event-log__entry-description">{event.description}</p>
        ...
        {filteredHistory.length === 0 && (
          <p className="event-log__empty-filtered-message">No {filter} events found in history.</p>
        )}
      </div>
    </div>
  );
