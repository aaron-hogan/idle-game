...
const EventCard: React.FC<EventCardProps> = ({ event, onChoiceSelected, onClose }) => {
  ...
  return (
    <Card
      className={`event-card event-card--${event.type.toLowerCase()} event-card--${event.category?.toLowerCase() || ''}`}
    >
      ...
      {(!event.choices || event.choices.length === 0) && onClose && (
        <div className="event-card__footer">
          <Button onClick={handleCloseClick} className="event-card__dismiss-button">
            Dismiss
          </Button>
        </div>
      )}
