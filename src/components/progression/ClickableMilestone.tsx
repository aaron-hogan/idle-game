...
const ClickableMilestone: React.FC<ClickableMilestoneProps> = ({
  ...
  const milestonesWithProgress = useMemo(() => {
    ...
    allMilestones.forEach((milestone) => {
      ...
      const overallProgress = resourceReqs.length > 0 ? totalProgress / resourceReqs.length : 0;
        ...
        {primaryReq && (
          <div className="primary-requirement">
            <div className="requirement-label">
              {primaryReq.resourceName}: {primaryReq.currentAmount.toFixed(1)} /{' '}
              {primaryReq.requiredAmount.toFixed(1)}
            </div>
            {primaryReq.timeToComplete !== null && primaryReq.timeToComplete > 0 && (
              <div className="eta">ETA: {formatTimeRemaining(primaryReq.timeToComplete)}</div>
            )}
          </div>
        )}
      </div>
