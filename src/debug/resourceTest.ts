    ...
    setTimeout(() => {
      ...
      initialValues.forEach((initial) => {
        ...
        if (updated) {
          ...
          console.log(
            `- ${initial.name}: ${initial.amount.toFixed(2)} → ${updated.amount.toFixed(2)} (change: ${change.toFixed(2)}, expected: ${expected.toFixed(2)})`
          );
    ...
    if (testResource) {
      ...
      setTimeout(() => {
        ...
        if (updatedResource) {
          ...
          console.log(
            `${testResource.name}: ${initialAmount.toFixed(2)} → ${newAmount.toFixed(2)} (change: ${change.toFixed(2)})`
          );
