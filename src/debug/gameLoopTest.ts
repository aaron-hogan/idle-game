...
export function runGameLoopDiagnostics() {
  ...
  Object.values(state.resources as Record<string, Resource>).forEach((resource) => {
    if (resource && resource.name) {
      console.log(
        `- ${resource.name}: amount=${resource.amount.toFixed(2)}, perSecond=${resource.perSecond.toFixed(2)}`
      );
    }
  });
  ...
  try {
    ...
    const startingValues = Object.values(state.resources as Record<string, Resource>).map(
      (resource) => ({
        id: resource.id,
        name: resource.name,
    ...
    setTimeout(() => {
      ...
      startingValues.forEach((startResource) => {
        ...
        if (endResource) {
          ...
          console.log(
            `- ${startResource.name}: changed by ${diff.toFixed(2)} (expected ${expected.toFixed(2)})`
          );
      ...
      if (shouldForceUpdate) {
        ...
        const testResource = Object.values(endState.resources as Record<string, Resource>).find(
          (r) => r.perSecond > 0
        );
        ...
        if (testResource) {
          ...
          setTimeout(() => {
            ...
            console.log(
              `${testResource.name} after manual update: ${finalResource.amount.toFixed(2)}`
            );
          }, 500);
        }
      }
