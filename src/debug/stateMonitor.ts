...
function takeSnapshot() {
  ...
  Object.entries(state.resources).forEach(([id, resource]) => {
    if (resource && typeof resource === 'object' && 'amount' in resource && 'perSecond' in resource) {
...
function detectChanges(latest: StateSnapshot): string[] {
  ...
  if (latest.gameTime !== previous.gameTime) {
    changes.push(`Game time changed: ${previous.gameTime.toFixed(1)}s → ${latest.gameTime.toFixed(1)}s`);
  }
  ...
  Object.entries(latest.resources).forEach(([id, resource]) => {
    ...
    if (prevResource) {
      ...
      if (Math.abs(resource.amount - prevResource.amount) > 0.01) {
        changes.push(`Resource ${id} amount changed: ${prevResource.amount.toFixed(2)} → ${resource.amount.toFixed(2)}`);
      }
      
      // Check for rate changes
      if (Math.abs(resource.perSecond - prevResource.perSecond) > 0.01) {
        changes.push(`Resource ${id} rate changed: ${prevResource.perSecond.toFixed(2)}/s → ${resource.perSecond.toFixed(2)}/s`);
      }
    }
  });
