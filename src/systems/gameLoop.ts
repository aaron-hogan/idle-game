...
export class GameLoop {
  ...
  private frameUpdate(timestamp: number): void {
    ...
    try {
      ...
      if (updatesThisFrame >= maxUpdatesPerFrame && this.accumulatedTime > tickDeltaTime) {
        console.log(
          `GameLoop: Too many updates needed, skipping ${this.accumulatedTime.toFixed(3)}s`
        );
      ...
      if (this.uiUpdateCount % 60 === 0) {
        const state = this.store!.getState();
        console.log(
          `GameLoop: UI frame #${this.uiUpdateCount}, deltaTime=${deltaTime.toFixed(4)}s, game time=${state.game.totalPlayTime.toFixed(2)}s`
        );
      }
  ...
  private fixedUpdate(deltaTime: number): void {
    try {
      ...
      if (this.tickCount % 10 === 0) {
        const state = this.store!.getState();
        console.log(
          `GameLoop: Tick #${this.tickCount}, delta=${deltaTime.toFixed(3)}s, game time=${state.game.totalPlayTime.toFixed(2)}s`
        );
      }
    } catch (error) {
      this.logger.logError(
  ...
  private tick(): void {
    ...
    try {
      ...
      console.log(
        `GameLoop: Tick ${this.tickCount}, actual=${elapsedSeconds.toFixed(3)}s, effective=${effectiveElapsed.toFixed(3)}s`
      );
      ...
      const state = this.store!.getState();
      console.log(
        `GameLoop: After tick, totalPlayTime=${state.game.totalPlayTime.toFixed(2)}s, lastSave=${calculateElapsedSecondsFromNow(state.game.lastSaveTime).toFixed(2)}s ago`
      );
    } catch (error) {
      this.logger.logError(
        error instanceof Error ? error : new Error(String(error)),
  ...
  private updateGameState(deltaTime: number): void {
    ...
    try {
      ...
      console.log(
        `LEGACY GameLoop: Not updating time, using new GameLoop instead. Delta: ${deltaTime.toFixed(3)}s`
      );
      ...
      if (this.tickCount % 10 === 0) {
        ...
        console.log(
          `LEGACY GameLoop: Game time: ${state.game.totalPlayTime.toFixed(2)}s, delta: ${deltaTime.toFixed(3)}s`
        );
      }
      ...
      if (this.tickCount % 100 === 0) {
        // Every 10 seconds at 10Hz
        const state = store.getState() as RootState;
        Object.values(state.resources).forEach((resource) => {
          if (resource && resource.name) {
            console.log(
              `Resource ${resource.name}: amount=${resource.amount.toFixed(2)}, perSecond=${resource.perSecond.toFixed(2)}`
            );
          }
        });
      }
  ...
  private processOfflineProgress(): void {
    ...
    try {
      ...
      console.log(
        `LEGACY GameLoop: Processing ${offlineSeconds.toFixed(1)}s of offline progress (resources only)`
      );
      ...
      if (offlineSeconds > 1) {
        ...
        console.log(
          `GameLoop: Added ${offlineSeconds.toFixed(1)}s to play time for offline progress`
        );
      } else {
  ...
  resume(): void {
    try {
      ...
      if (pauseDuration > LONG_PAUSE_THRESHOLD && this.config.processOfflineProgress) {
