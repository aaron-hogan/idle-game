...
export class ResourceManager {
  ...
  public static getInstance(
    dependenciesOrStore?: ResourceManagerDependencies | { dispatch: unknown; getState: unknown }
  ): ResourceManager {
    if (!ResourceManager.instance) {
      if (!dependenciesOrStore) {
        ...
        ResourceManager.instance = new ResourceManager(
          dependenciesOrStore as ResourceManagerDependencies
        );
      } else {
  ...
  initializeResources(): void {
    ...
    Object.values(INITIAL_RESOURCES).forEach((resource) => {
      if (
        validateObject(
          resource,
          ['id', 'name', 'amount', 'perSecond', 'unlocked'],
          'initializeResources'
        )
      ) {
  ...
  updateResources(gameTimeInSeconds: number): void {
    ...
    const oppression =
      'byId' in resources
        ? ((resources.byId as unknown as Record<string, unknown>)[ResourceId.OPPRESSION] as {
            perSecond: number;
            id: string;
          })
        : ((resourcesObj as Record<string, unknown>)[ResourceId.OPPRESSION] as {
            perSecond: number;
            id: string;
          });
    ...
    if (oppression) {
      ...
      if (oppression.perSecond !== OPPRESSION_RATE) {
        this.dispatch(
          this.actions.updateResourcePerSecond({
      ...
      if (oppressionAmount > 0) {
        this.dispatch(
          this.actions.addResourceAmount({
            id: ResourceId.OPPRESSION,
    ...
    const resourceValues = resources.byId
      ? Object.values(resources.byId)
  ...
  calculateResourceGeneration(): void {
    ...
    Object.values(structures).forEach((structure: unknown) => {
      ...
      if (
        ...
        !('maxWorkers' in structure) ||
        !('production' in structure) ||
        structure.unlocked !== true ||
        typeof structure.level !== 'number' ||
        structure.level <= 0
      ) {
        return;
      }
      ...
      if (structure.production && typeof structure.production === 'object') {
        ...
        Object.entries(structure.production as Record<string, unknown>).forEach(
    ...
    Object.keys(baseRates).forEach((resourceId) => {
      ...
      if (typeof baseRate === 'number' && !isNaN(baseRate)) {
        ...
        if (resource) {
          ...
          // Update base rate
          this.dispatch(
            this.actions.updateBaseResourcePerSecond({
              id: resourceId,
  ...
  unlockResource(resourceId: string): boolean {
    ...
    this.dispatch(
      this.actions.toggleResourceUnlocked({
        id: resourceId,
        unlocked: true,
      })
    );
  ...
  setResourceAmount(resourceId: string, amount: number): boolean {
    ...
    // Set the amount
    this.dispatch(
      this.actions.updateResourceAmount({
        id: resourceId,
        amount: amount,
      })
    );
  ...
  upgradeClickPower(resourceId: string): boolean {
    ...
    this.dispatch(
      this.actions.updateUpgradeLevel({
        id: resourceId,
