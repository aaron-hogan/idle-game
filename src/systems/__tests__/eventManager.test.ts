...
describe('EventManager', () => {
  ...
  test('should resolve an event with a choice', () => {
    ...
    const testEvent = createTestEvent({
      choices: [{ id: 'test-choice', text: 'Test Choice' }],
    });
  ...
  test('should heal event state inconsistencies', () => {
    ...
    const isFixed =
      (state.events.activeEvents.includes('inconsistent-event') &&
        fixedEvent.status === EventStatus.ACTIVE) ||
      (!state.events.activeEvents.includes('inconsistent-event') &&
        fixedEvent.status === EventStatus.PENDING);
