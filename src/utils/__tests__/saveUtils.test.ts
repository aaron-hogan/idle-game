...
describe('saveUtils', () => {
  ...
  describe('loadGame', () => {
    ...
    it('should log warning when version mismatch', () => {
      ...
      // Assert
      expect(result).toEqual(mockSaveData);
      expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Save version mismatch'));
  ...
  describe('backupSave', () => {
    it('should create a backup of the current save', () => {
      ...
      // Assert
      expect(result).toBe(true);
      expect(localStorageMock.setItem).toHaveBeenCalledWith(BACKUP_SAVE_KEYS[0], saveData);
    });
