...
export const assignWorkersToBuilding =
  (buildingId: string, workers: number): AppThunk =>
  (dispatch, getState) => {
    try {
...
export const changeWorkers =
  (buildingId: string, delta: number): AppThunk =>
  (dispatch, getState) => {
    try {
...
export const autoAssignWorkers =
  (strategy: 'balanced' | 'focused' | 'efficiency' = 'balanced'): AppThunk =>
  (dispatch, getState) => {
    try {
      // Validate strategy
