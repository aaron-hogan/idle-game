...
if [ -z "$1" ]; then
  ...
  PR_INFO=$(gh pr list --head "$CURRENT_BRANCH" --json number,title,state,url,reviewDecision,checks --jq '.[0]')
  ...
  PR_INFO=$(gh pr view "$1" --json number,title,state,url,reviewDecision,checks --jq '.')
fi
